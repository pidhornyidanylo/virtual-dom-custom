...
function diffProps(newNode, oldNode) {
    ...
    Object.keys(props).forEach(name => {
        ...
        if(!newVal) {
            patches.push({ type: REMOVE_PROP, name, value: oldNode })
        } else if(!oldValue || newVal !== oldValue) {
...
function diff(newNode, oldNode) {
    ...
    if(newNode.type) {
        return { type: UPDATE, children: diffChildren(newNode, oldNode), props: diffProps(newNode, oldNode) }
    }
}
...
function patchProps(parent, patches) {
    for(let i = 0; i < patches.length; i ++) {
        const propsPatch = patches[i]
        const { type, name, value } = propsPatch;
        if(type === SET_PROP) {
